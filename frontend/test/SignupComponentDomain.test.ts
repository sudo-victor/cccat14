...
import SignupComponentDomain from "../src/domain/SignupComponentDomain.ts"

test("Deve testar o fluxo do wizard", async function () {
  const signupComponent = new SignupComponentDomain()
  const name = "John Doe"
  const email = `john.doe_${Math.random()}@gmail.com`
...
test("Nao deve ir para o passo 2 se pelo menos uma opçao(passenger ou driver) nao estiver marcada", async function () {
  const signupComponent = new SignupComponentDomain()
  expect(signupComponent.step).toBe(1)
  signupComponent.next()
...
test("Nao deve ir para o passo 3 se os campos: nome, email, cpf e placa do carro nao estiverem preenchidos", async function () {
  const signupComponent = new SignupComponentDomain()
  const name = "John Doe"
  const email = `john.doe_${Math.random()}@gmail.com`
  const cpf = "97456321558"
